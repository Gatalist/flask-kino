...
from app.movies.models import Movies, Genres
...
class MoviesAPI(MixinMovie, MethodResource, Resource):
    ...
    def get(self):
        return self.responce_data(Movies, MoviesSchema)
    ...
    def post(self, **kwargs):
        ...
        print('movie', movie)
        Genres.genre_add_to_movie(movie.id, kwargs["genres"])
...
class MoviesAPI2(MixinMovie, MethodResource, Resource):
    ...
    def put(self, movie_id, **kwargs):
        ...
        if movie:
            ...
            Genres.genre_dell_to_movie(movie_id)
            Genres.genre_add_to_movie(movie.id, kwargs["genres"])
            return self.response_movie(movie)
        return error_response(301, "Not found movie")
    ...
    def delete(self, movie_id):
        movie = Movies.query.filter_by(id=int(movie_id)).first()
        if movie:
            Genres.genre_dell_to_movie(movie_id)
